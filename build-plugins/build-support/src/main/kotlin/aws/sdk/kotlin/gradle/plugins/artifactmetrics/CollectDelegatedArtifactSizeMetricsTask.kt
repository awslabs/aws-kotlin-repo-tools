...
internal abstract class CollectDelegatedArtifactSizeMetricsTask : DefaultTask() {
    ...
    fun collect() {
        ...
        artifactSizeMetricsFileKeys.filter { it.startsWith("[TEMP]${project.rootProject.name}-$identifier-") }
    ...
    private fun getArtifactSizeMetricsFileKeys(): List<String>? = runBlocking {
        S3Client.fromEnvironment().use { s3 ->
            return@runBlocking s3.listObjects {
                bucket = ""
            }.contents?.map { it.key ?: throw AwsSdkGradleException("Missing key for object $it") }
        }
    }
